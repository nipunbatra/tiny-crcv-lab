...
function methodCost(key: FreshMethodKey, metrics: Record<ModelKind, FreshComparisonMetrics>) {
  const meanSeconds = (metrics.instruct.methods[key].incremental_runtime.mean_seconds_per_question + metrics.base.methods[key].incremental_runtime.mean_seconds_per_question) / 2;
  ...
  return <section className="border-b hairline bg-[#eae6dc] py-16">
    <div className="shell">
      <div className="grid gap-5 lg:grid-cols-[1fr_.72fr] lg:items-end">
        <div>
          <p className="eyebrow">Fresh 600-question evidence</p>
          <h2 className="mt-2 max-w-4xl text-4xl font-semibold tracking-[-.05em]">Start simple. Then inspect every alternative.</h2>
        </div>
        <p className="text-sm leading-6 text-[#69716d]">The original five-method comparison remains frozen. A clearly marked post-hoc extension now audits CRCV, all 24 saved trace scalars, two low-budget literature-inspired consistency scores, and two tiny multifeature models.</p>
      </div>
      <div className="mt-7 grid gap-4 lg:grid-cols-3">
        <div className="bg-[#18211d] p-5 text-white"><p className="eyebrow !text-[#bfc7c2]">Default</p><p className="mt-3 text-xl font-semibold">Top-3 token surprise</p><p className="mt-2 text-sm leading-6 text-[#bfc7c2]">One pass, transparent, and the most defensible frozen recommendation.</p></div>
        <div className="bg-[#fffdf8] p-5"><p className="eyebrow">If three samples fit</p><p className="mt-3 text-xl font-semibold">Lexical disagreement</p><p className="mt-2 text-sm leading-6 text-[#69716d]">Highest observed Instruct AUROC: <span className="mono">{format(metrics.instruct.methods.lexical_disagreement_3.held_out.auroc)}</span>. Exploratory, not confirmed.</p></div>
        <div className="bg-[#fffdf8] p-5"><p className="eyebrow">Base checkpoint</p><p className="mt-3 text-xl font-semibold">Maximum ambiguity</p><p className="mt-2 text-sm leading-6 text-[#69716d]">Highest observed Base AUROC: <span className="mono">{format(metrics.base.methods.token_ambiguity_max.held_out.auroc)}</span>. Also exploratory.</p></div>
      </div>
      <div className="card mt-5 overflow-x-auto">
        <table className="w-full min-w-[840px] text-left text-sm">
          <thead className="bg-[#eeeae1] text-[11px] uppercase tracking-[.07em] text-[#69716d]"><tr><th className="p-4">Representative baseline</th><th className="p-4">Instruct AUROC</th><th className="p-4">Base AUROC</th><th className="p-4">Online cost</th></tr></thead>
          <tbody>{overviewOrder.map((key) => <tr className="border-t hairline" key={key}>
            <td className="p-4"><p className="font-semibold">{metrics.instruct.methods[key].display_name}</p><p className="mt-1 text-xs text-[#69716d]">{methodRole(key, metrics.instruct)}</p></td>
            <td className="mono p-4 text-lg">{format(metrics.instruct.methods[key].held_out.auroc)}</td>
            <td className="mono p-4 text-lg">{format(metrics.base.methods[key].held_out.auroc)}</td>
      ...
      <div className="mt-5 grid gap-4 lg:grid-cols-[1.25fr_.75fr]">
        <div className="border-l-4 border-[#df4c2f] bg-[#fff8f4] p-5 text-sm leading-6"><strong>Statistical reading.</strong> None of the added methods has a paired 95% AUROC-difference interval entirely above zero versus top-3 surprise. “Best observed” is a lead for a new confirmatory run, not a win.</div>
...
function ProbeAudit({ prediction }: { prediction: FreshPrediction }) {
  ...
  return <AuditCard name="Mean-hidden linear probe" score={prediction.method_scores.hidden_logistic_probe} formula="sigmoid(b + Σ wⱼzⱼ)"><p>Average the final 896-value hidden state across answer-token decisions, standardize each dimension on calibration only, and take one weighted sum.</p><div className="mt-3 overflow-x-auto"><table className="w-full min-w-[680px] text-left"><thead className="text-[#69716d]"><tr><th>dimension</th><th>hⱼ</th><th>zⱼ</th><th>wⱼ</th><th>wⱼzⱼ</th></tr></thead><tbody>{probe.top_contributions.map((item) => <tr className="border-t hairline" key={item.dimension}><td className="mono py-2">{item.dimension}</td><td className="mono">{format(item.raw_hidden, 4)}</td><td className="mono">{format(item.standardized_hidden, 4)}</td><td className="mono">{signed(item.weight, 4)}</td><td className="mono">{signed(item.contribution, 4)}</td></tr>)}</tbody></table></div><p className="mono mt-3">logit = bias {signed(probe.bias, 5)} + shown {signed(probe.top_contributions.reduce((sum, item) => sum + item.contribution, 0), 5)} + other 888 dims {signed(probe.other_dimensions_contribution, 5)} = {format(probe.logit, 5)}</p><p className="mono mt-2">sigmoid = <strong>{format(probe.sigmoid_risk, 5)}</strong></p></AuditCard>;
}
...
function FreshQuestionInspector({ model, metrics }: { model: ModelKind; metrics: FreshComparisonMetrics }) {
  ...
  useEffect(() => {
    ...
    setRows([]);
    fetch(`./data/fresh_qa_qwen05b_${model}.jsonl`).then((response) => { if (!response.ok) throw new Error(`Trace download failed (${response.status})`); return response.text(); }).then((text) => { const loaded = parseJsonl<FreshPrediction>(text); traceCache[model] = loaded; if (!cancelled) setRows(loaded); }).catch((reason) => { if (!cancelled) setError(String(reason)); });
...
export function FreshBenchmark({ model, metrics }: { model: ModelKind; metrics: Record<ModelKind, FreshComparisonMetrics> }) {
  ...
  return <section className="shell py-16"><div><p className="eyebrow">Frozen result + post-hoc extension</p><h2 className="mt-2 text-4xl font-semibold tracking-[-.05em]">Fresh public QA comparison.</h2><p className="mt-3 max-w-3xl text-sm leading-6 text-[#69716d]">AUROC asks how often one wrong answer receives higher risk than one correct answer. AURAC asks how accurate the retained answers remain as we reject higher-risk answers. Select a method for its exact formula and operating point.</p></div>
    <div className="mt-5 border-l-4 border-[#df4c2f] bg-[#fff8f4] p-4 text-sm leading-6"><strong>Evidence boundary.</strong> The five original methods were frozen before held-out evaluation. The 26 additional views were specified only after those held-out outputs had been inspected, so their rankings are exploratory even though their formulas were frozen before this aggregate rerun.</div>
